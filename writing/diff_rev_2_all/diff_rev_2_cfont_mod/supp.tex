...
%DIF 38a43-44
 %DIF > 
\newcommand{\STAB}[1]{\begin{tabular}{@{}c@{}}#1\end{tabular}} %DIF > 
%DIF -------
...
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1.15\textwidth]{figure_s2}\DIFaddendFL }
    \vspace{-4cm}
    \caption{Isocratic $k'$ data that were consolidated from the literature. Each series represents a unique protein-pH-resin combination, and lines represent quasi-SDM fits to the data. \DIFaddbeginFL \DIFaddFL{These data, which are available in the \texttt{Supplementary\_table\_S2.xlsx} file, were acquired by digitizing plots (using the Engauge Digitizer software), which may introduce some error into the precise $k'$ values.}\DIFaddendFL }
