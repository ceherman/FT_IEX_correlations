...
def get_sim_results(results_folder, file, flow_flag=True):
    ...
    Ds     = sim.root.input.model.unit_001.par_surfdiffusion[0]
    d_part = sim.root.input.model.unit_001.par_radius * 2.0
    l_col  = sim.root.input.model.unit_001.col_length
    ...
    try:
        ...
        keq   = ka/kd
    if flow_flag:
        return c_load, dp, ka, q_max, kd, keq, t, c_out, fVolum, Ds, d_part, l_col
    else:
        return c_load, dp, ka, q_max, kd, keq, t, c_out, Ds, d_part, l_col

def get_sim_results_more_general(results_folder, file):
    sim    = get_sim_results_file(results_folder, file)
    c_load = sim.root.input.model.unit_000.sec_000.const_coeff[0]
    Dp     = sim.root.input.model.unit_001.par_diffusion[0]
    t      = sim.root.output.solution.solution_times
    ...
    # kd     = sim.root.input.model.unit_001.adsorption.mcl_kd[0]
    # keq    = q_max*ka/kd
    return c_load, c_out, t, fVolum, Dp, d_part, l_col, Ds
...
def set_layout(fig, ax, legend=False, fontsize=20, loc='best'):
    if legend:
        ax.legend(loc=loc, handlelength=1, frameon=False, fontsize=fontsize)
